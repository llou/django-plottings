...
class ValueMixin:
    """
    This mixin provides ``BasePlot`` with the ``get_value()`` method that
    returns a safe string to be used inside a template. It also has the magic
    method ``__str__`` so it can be injected to the template.
    """
...
class PNGBase64PlotToValue(Base64ValueMixin, PNGPlotMixin, BasePlot):
    """
...
class CachedPNGBase64PlotToValue(CachedMixin, Base64ValueMixin, PNGPlotMixin, BasePlot):
    """
    This class is used to dump a PNG figure encoded in Base64 format into a
    variable ready to be passed to the template engine to render it in a
